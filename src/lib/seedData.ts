...
const projectsRaw = [
  { id: "p1", name: "Clinic Workflow Tracker", short_description: "Operations tracking app for a small healthcare team", status: "stalled", project_type: "web_app", platform_name: "Lovable", platform_logo_slug: "lovable", account_label: "Primary", last_active_date: daysAgo(11), next_action: "Redesign the core data model before writing any more UI", blocker_summary: "Got auth working but the schema went sideways when I added the workflow module. The core relationships need a rethink before going further.", confidence_score: 7, motivation_score: 8, monetisation_score: 9, platform_fit_score: 8, effort_score: 6, priority: 2 },
  { id: "p2", name: "Neighbourhood Health Site Revamp", short_description: "Full site rebuild for a local health business", status: "building", project_type: "website_revamp", platform_name: "VS Code + Copilot", platform_logo_slug: "visualstudiocode", account_label: "Primary", last_active_date: daysAgo(2), next_action: "Finish mobile layout for the services section", blocker_summary: "Content is mostly done. Mobile layout still needs work on the services section and the hero image crops badly on 375px.", confidence_score: 9, motivation_score: 7, monetisation_score: 8, platform_fit_score: 9, effort_score: 4, priority: 1 },
  { id: "p3", name: "Supplier Price Scraper", short_description: "Scrapes supplier catalogues for pricing checks", status: "stalled", project_type: "scraping_tool", platform_name: "Replit", platform_logo_slug: "replit", account_label: "Ops", last_active_date: daysAgo(18), next_action: "Try a Playwright-based approach instead of requests+BeautifulSoup", blocker_summary: "The base scraper works fine for simple pages but falls over on paginated supplier catalogues.", confidence_score: 6, motivation_score: 5, monetisation_score: 7, platform_fit_score: 6, effort_score: 7, priority: 3 },
  { id: "p4", name: "Service Form Builder", short_description: "Dynamic form builder for internal service workflows", status: "paused", project_type: "web_app", platform_name: "Base44", account_label: "Ops", last_active_date: daysAgo(22), next_action: "Either top up Base44 credits or migrate to Lovable with the ops account", blocker_summary: "Ran out of Base44 credits about two-thirds through. The core builder works, the PDF export is half-done.", confidence_score: 7, motivation_score: 6, monetisation_score: 8, platform_fit_score: 5, effort_score: 5, priority: 3 },
  ...
  { id: "p8", name: "Internal Team Scheduler", short_description: "Shift scheduling tool for a small team with leave and availability tracking", status: "planning", project_type: "web_app", platform_name: "v0", account_label: "Primary", last_active_date: daysAgo(6), next_action: "Finalise the data model before starting the UI build", blocker_summary: null, confidence_score: 6, motivation_score: 5, monetisation_score: 6, platform_fit_score: 6, effort_score: 6, priority: 3 },
  { id: "p9", name: "Demo Project Tracker", short_description: "This app, tracking its own development in demo form.", status: "building", project_type: "web_app", platform_name: "Lovable", platform_logo_slug: "lovable", account_label: "Primary", last_active_date: daysAgo(0), next_action: "Get the stall diagnosis workflow properly wired to the mock AI", blocker_summary: null, confidence_score: 9, motivation_score: 9, monetisation_score: 7, platform_fit_score: 9, effort_score: 3, priority: 1 },
  { id: "p10", name: "AI Workflow Sandbox", short_description: "Local automation setup for chaining prompts across coding tasks", status: "stalled", project_type: "automation", platform_name: "Cursor", platform_logo_slug: "cursor", account_label: "Primary", last_active_date: daysAgo(9), next_action: "Write up the current architecture properly before changing anything else", blocker_summary: "The individual pieces work but the prompt chaining is inconsistent.", confidence_score: 6, motivation_score: 7, monetisation_score: 5, platform_fit_score: 8, effort_score: 6, priority: 2 },
];

export const seedProjects: SeedProject[] = projectsRaw.map((p) => withDerivedProjectFields(p));

export const seedAccounts: SeedAccount[] = [
  { id: "a1", label: "Primary", email_address: "primary@example.dev", login_method: "Google", preferred_use_case: "Main builds, Supabase, Vercel deployments", best_for_platform: "Lovable", is_active: true, last_used_date: daysAgo(0) },
  { id: "a2", label: "Ops", email_address: "ops@example.dev", login_method: "Google", preferred_use_case: "Replit, Base44, Bolt free tiers for internal tools", best_for_platform: "Replit", is_active: true, last_used_date: daysAgo(4) },
  { id: "a3", label: "Alt Dev", email_address: "alt-dev@example.dev", login_method: "email", preferred_use_case: "Fresh credits for experiments and prototypes.", best_for_platform: "Manus", is_active: true, last_used_date: daysAgo(19) },
];
...
export const seedUpdates: SeedUpdate[] = [
  ...
  { id: "u2", project_name: "Demo Project Tracker", type: "note", content: "Got the dashboard KPI cards rendering with live data from storage. Next: rescue ranking panel.", created_at: new Date(today.getTime() - 4 * 3600000).toISOString() },
  { id: "u3", project_name: "Neighbourhood Health Site Revamp", type: "status_change", content: "Status changed to building after the latest content review.", created_at: new Date(today.getTime() - 26 * 3600000).toISOString() },
  { id: "u4", project_name: "Internal Team Scheduler", type: "note", content: "Sketched out the shift model. Need to handle split shifts and on-call separately.", created_at: new Date(today.getTime() - 48 * 3600000).toISOString() },
  { id: "u5", project_name: "AI Workflow Sandbox", type: "note", content: "Tried reversing the chain order, slightly better but still not consistent enough.", created_at: new Date(today.getTime() - 72 * 3600000).toISOString() },
  { id: "u6", project_name: "Clinic Workflow Tracker", type: "assessment", content: "Ran stall diagnosis. Main issue is schema complexity, not motivation. Rescue score: 68.", created_at: new Date(today.getTime() - 168 * 3600000).toISOString() },
...
export const seedCosts = [
  ...
  { id: "c2", platform: "Base44", amount: 12, currency: "AUD", entry_type: "one_off", date: daysAgo(14), account_label: "Ops", project_name: "Service Form Builder", notes: "Promo credit top-up" },
  { id: "c3", platform: "Replit", amount: 8, currency: "AUD", entry_type: "subscription", date: daysAgo(3), account_label: "Ops", project_name: null, notes: "Hacker plan" },
];
